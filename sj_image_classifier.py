...
"""Shared model, training loop and reporting code for the SparseVQCore image
classification tests (image_classification_test.py, fashion_classification_test.py).

Proves that the SparseVQCore VSA bottleneck (base.py) is domain-agnostic: no
video, no physics, just a small CNN image encoder feeding SparseVQCore, with
...
def run_classification_test(name: str, train_ds: Dataset, test_ds: Dataset, class_names: list[str],
    ...
    spec_text = specialization_report_text(all_codes, all_targets, physical_edges(masks), cfg.card)
